...
@inline function gooddivrem(x::T,y::Int64)::Tuple{Int,T} where T
    a,b = divrem(x,y)
    return Int(a), b
end
...
function fast_trilinear_earth_interpolate(u::SVector{2,T},p,tin::Float64) where {T<:Real}
    Us = p[1]
    Vs = p[2]
    nx::Int64 = size(Us)[1]
    ny::Int64 = size(Us)[2]
...
@inbounds @inline function base_tricubic_interpolation(
    ...
    for k in 0:1, j in 0:1, i in 0:1
        ...
        for r in  A.colptr[aindex]:(A.colptr[aindex + 1] - 1)
            ...
            yValIndex::Int64 = ( (rowIndex - xValIndex)/4) % 4
            tValIndex::Int64 = ( (rowIndex - xValIndex)/4 - yValIndex) / 4
...
function inbounds_checker_bilinear(x,y,p)
    du = [0.0,0.0]
    fast_trilinear_earth_interpolate(du,(x,y),p[6],p[6][5][1])
    return !isnan(du[1])
end
